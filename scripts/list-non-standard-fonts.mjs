...
async function listNonStandardFonts() {
  ...
  if (nonStandard.length === 0) {
    console.log(
      chalk.green('✅ All fonts in the assets folder are correctly dated!'),
    );
  } else {
    ...
    console.log(
      chalk.yellow(
        `🚩 Found ${nonStandard.length} fonts missing date prefixes. List saved to: ${chalk.white(reportPath)}`,
      ),
    );
