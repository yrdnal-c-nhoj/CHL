...
const VenusClock: React.FC = () => {
    ...
    export default function VenusClock() {
        ...
        // --- Preload font + images ---
        useEffect(() => {
            const styleEl: React.CSSProperties = document.createElement('style');
  ...
  const backgroundLayerStyle = (url: string, scale: number): React.CSSProperties => ({
    position: 'absolute',
    left: '50%',
    top: '50%',
    ...
    const backgroundLayerStyle = (url, scale): React.CSSProperties => ({
        position: 'absolute',
        left: '50%',
        top: '50%',
        transform: `translate(-50 %, -50 %) scale(${ scale })`,
  ...
  const tickCommon = {
    ...
    const numberStyle = (i): React.CSSProperties => {
