    ...
    useEffect(() => {
        ...
        const fetchTime = async () => {
            try {
                ...
                if (isMounted) {
                    setOffset(
                        new Date(data.utc_datetime).getTime() - (Date.now() + networkDelay),
                    );
                    setIsSynced(true);
                }
            } catch (e) {
...
export const fontConfigs: FontConfig[] = [
    {
        ...
        fontUrl: fontClockUrl,
        options: { display: 'swap' },
    },
    {
        fontFamily: 'MarqueeFont',
        fontUrl: fontMarqueeUrl,
        options: { display: 'swap' },
    },
];
const NtpClock: React.FC = () => {
    ...
    return (
        ...
        >
            ...
            >
                {`${displayTime} NTP is a system that keeps computers accurate. `.repeat(
                    5,
                )}
            </div>
        </div>
