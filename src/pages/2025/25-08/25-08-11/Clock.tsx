...
const SwirlingImages: React.FC = () => {
    ...
    const clockNumberStyle: React.CSSProperties = {
        position: 'absolute',
        color: '#D4E7F7FF',
        fontFamily: '"CustomFont", sans-serif',
        fontSize: '4.2rem',
    ...
    const getOrbitWrapperStyle = (img): React.CSSProperties => ({
        position: 'absolute',
    ...
    const getInnerWrapperStyle = (img): React.CSSProperties => ({
        animation: `wobble-${img.id} ${img.wobbleSpeed}s ease-in-out infinite alternate`,
    });

    const getImageStyle = (img): React.CSSProperties => ({
        width: `${img.size}rem`,
        height: `${img.size}rem`,
        borderRadius: '0.8rem',
        objectFit: 'cover',
