  ...
  useEffect(() => {
    ...
    const updateClock = (timestamp) => {
      ...
      if (digitStatesRef.current.length === 0) {
        ...
        digitStatesRef.current = timeStr
          .split('')
          .map((d) => ({ currentDigit: d, targetDigit: d, t: 1 }));
      }
      ...
      // Draw background
      const bgGrad = createShiftingGradient(
        ctx,
        0,
        0,
        canvas.width,
      ...
      const totalW =
        displayChars.length * digitW + (displayChars.length - 1) * spacing;
      ...
      displayChars.forEach((char, i) => {
        ...
        const path =
          digitPaths[state.currentDigit]?.(x, y, digitW, digitH) ?? [];
        const target =
          digitPaths[state.targetDigit]?.(x, y, digitW, digitH) ?? path;
        const grad = createShiftingGradient(
          ctx,
          x,
          y,
          x,
