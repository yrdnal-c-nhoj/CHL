...
const DynamicComponent = () => {
  ...
  return (
    <main className={styles.container}>
      <time dateTime={clockTime.toISOString()} className={styles.srOnly}>
        {clockTime.toLocaleTimeString()}
      </time>
      <div className={styles.clockWrapper}>
        <div
          className={styles.horizontalClock}
          style={{
            fontFamily: 'TopFont',
            textShadow:
              '-0px 34.5dvh 1.7dvh rgba(0, 0, 0, 0.9), 0px 2px 12px rgb(240, 7, 7)',
          }}
        >
          {timeString.split('').map((d, i) => (
            <div key={`l-${i}`} className={styles.digitBox}>
              {d}
            </div>
          ))}
        </div>
      ...
      <div className={styles.clockWrapper}>
        <div
          ...
          style={{
            ...
            textShadow:
              '0px -34.5dvh 1.7dvh rgba(0, 0, 0, 0.9), 0px -2px 12px rgb(238, 9, 9)',
          }}
        >
          {timeString.split('').map((d, i) => (
            <div key={`r-${i}`} className={styles.digitBox}>
              {d}
            </div>
          ))}
        </div>
