...
const Clock = () => {
  ...
  useEffect(() => {
    ...
    const loadImages = async () => {
      try {
        ...
        const imageModules = import.meta.glob('/src/assets/images/26-02/26-02-26/*.{png,jpg,jpeg,gif,svg,webp}', { eager: true });
  ...
  useEffect(() => {
    if (images.length > 0 && gridSize.rows > 0 && gridSize.cols > 0) {
      ...
      // If we need more images than available, repeat images
  ...
  const handleCornerImageLoad = () => {
    console.log('Corner image loaded');
    setCornerImageLoaded(true);
  };

  const handleCornerImageError = () => {
    console.error('Error loading corner image');
  };
  ...
  const cornerImageStyle = {
    ...
    objectFit: 'cover',
    zIndex: 15,
    border: '2px solid rgba(254, 255, 254, 0.8)',
    borderRadius: '8px',
