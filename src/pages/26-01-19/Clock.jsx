...
const COLORS = {
  ...
  mainHands: '#1E293B',   
  border: '#330202',      // Black outline
};

// --- Physics deviation functions (RESTORED TO FIX CRASH) ---
...
const ComplexYellowHand = ({ rotation, zIndex, transition = 'none', size }) => {
  ...
  return (
    <div style={{
      ...
      {/* Main blade (Yellow + Black Border) */}
      <div style={{
...
const ManyHandClock = () => {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      // Panicked Logic (Stuck 0-6s, realizes 7s, stays +2s ahead with shake)
      if (s >= 0 && s < 6) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', width: `${clockSize}vh`, height: `${clockSize}vh` }}>
        ...
        <ComplexYellowHand rotation={baseSecondRot} size={clockSize} zIndex={100} transition="none" />
